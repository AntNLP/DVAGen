document.addEventListener("DOMContentLoaded", () => {
	...
	async function FetchTokensFromServer(prefix) {
		...
		alterTitle.innerHTML = "DVAModel is generating response...";
	...
	function AlternativeBox(token, percentage, type, selectedTokenIndex) {
		...
		optionBox.addEventListener("click", async () => {
			await handleAlternativeSelection(selectedTokenIndex, token, type);
		});
	...
	function renderCurrentTokens(selectedTokenIndex) {
		...
		alterTitle.innerHTML = "Click the token or phrase above to view alternative candidates.";
	...
	function displayAlternatives(alternatives, selectedTokenIndex) {
		...
		alterTitle.textContent = `Current selected token/phrase: "${currentChosenTokenText}"`;
	...
	async function handleAlternativeSelection(selectedTokenIndex, selectedToken, selectedTokenType) {
        ...
        currentTokenSequence[selectedTokenIndex].type = selectedTokenType;
		let selectedTokenItem = outputBox.querySelector(
			`:nth-child(${selectedTokenIndex + 1})`
		);
		selectedTokenItem.innerHTML = selectedToken;
        selectedTokenItem.classList.remove("token");
        selectedTokenItem.classList.remove("phrase");
        selectedTokenItem.classList.add(selectedTokenType);
	...
	exampleButton.addEventListener("click", async () => {
		inputBox.value = "In recent years, artificial intelligence has become increasingly important in a wide range of industries.";
		// phraseTable.value = "cutting-edge technology, make a difference, take into account, on the same page, in the long run, as a matter of fact, by no means, in light of recent events, at the end of the day, a blessing in disguise";
		phraseTable.value = "";
